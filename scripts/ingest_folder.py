...
class LocalIngestWorker:
    ...
    async def ingest_folder(self, folder_path: Path, ignored: list[str]) -> None:
        # Count total documents before ingestion
        self._find_all_files_in_folder(folder_path, ignored)
        await self._ingest_all(self._files_under_root_folder)

    async def _ingest_all(self, files_to_ingest: list[Path]) -> None:
        logger.info("Ingesting files=%s", [f.name for f in files_to_ingest])
        await self.ingest_service.async_bulk_ingest([(str(p.name), p) for p in files_to_ingest])

    async def ingest_on_watch(self, changed_path: Path) -> None:
        logger.info("Detected change in at path=%s, ingesting", changed_path)
        await self._do_ingest_one(changed_path)

    async def _do_ingest_one(self, changed_path: Path) -> None:
        try:
            if changed_path.exists():
                logger.info(f"Started ingesting file={changed_path}")
                await self.ingest_service.ingest_file(changed_path.name, changed_path)
                logger.info(f"Completed ingesting file={changed_path}")
        except Exception:
            logger.exception(
                f"Failed to ingest document: {changed_path}, find the exception attached"
...
async def main():
    ...
    settings = global_injector.get(Settings)
    worker = LocalIngestWorker(ingest_service, settings)
    await worker.ingest_folder(root_path, args.ignored)
