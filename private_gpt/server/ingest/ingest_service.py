...
class IngestService:
    ...
    async def _ingest_data(self, file_name: str, file_data: AnyStr) -> list[IngestedDoc]:
        ...
        async def async_ingest():
            with tempfile.NamedTemporaryFile(delete=False) as tmp:
                try:
                    ...
                    return await self.ingest_file(file_name, path_to_tmp)
                finally:
                    tmp.close()
                    path_to_tmp.unlink()
    ...
    async def ingest_file(self, file_name: str, file_data: Path) -> list[IngestedDoc]:
        logger.info("Ingesting file_name=%s", file_name)
        documents = await self.ingest_component.async_ingest(file_name, file_data)
    ...
    async def ingest_bin_data(
        ...
        return await self._ingest_data(file_name, file_data)
...
#         logger.info("Ingesting file_names=%s", [f[0] for f in files])
#         documents = await self.ingest_component.bulk_ingest(files) #todo check the async 
